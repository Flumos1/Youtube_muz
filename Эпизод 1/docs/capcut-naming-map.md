# CapCut Naming Map — Fleetwood Mac / Rumours Pilot

**Rule:** filename = start timecode in MMSS format. Each image = 5 seconds on the CapCut timeline.  
**Total:** 46 frames × 5s = 3:50 of image coverage.

---

## Frame Map

| File | Timecode | Narrative Block | Scene | Voiceover cue |
|------|----------|-----------------|-------|---------------|
| 0000.png | 0:00 | Hook | Dark studio, mixing board with lamp, tape reels | "lock themselves inside a studio by the bay" |
| 0005.png | 0:05 | Hook | Two couple-silhouettes with backs to each other | "two of the couples… have just broken up" |
| 0010.png | 0:10 | Hook | Solitary silhouette at the board, ring on hand | "a third man is losing his marriage" |
| 0015.png | 0:15 | Hook | Live room, instruments, figures turned away | "make an album about love… can't look at each other" |
| 0020.png | 0:20 | Open Loop | Lips at vintage microphone | "made their exes sing them" |
| 0025.png | 0:25 | Open Loop | Hand writing on paper under lamp | "writing songs about each other" |
| 0030.png | 0:30 | Open Loop | Trembling VU-meter needle | "how that didn't tear them apart" |
| 0035.png | 0:35 | Bit 1 | Concert from backstage, sea of silhouettes | "the version everyone remembers came together" |
| 0040.png | 0:40 | Bit 1 | Tour bus on night highway | "overnight, they were global stars" |
| 0045.png | 0:45 | Bit 1 | Wall of gold records | "sold-out shows, real money, perfect picture" |
| 0050.png | 0:50 | Bit 1 | 5 silhouettes gathering / the band forming | Band forms, united front |
| 0055.png | 0:55 | Bit 1 | Drummer at kit, focused | Mick Fleetwood energy |
| 0100.png | 1:00 | Bit 1 | Guitarist and singer facing each other | Lindsey & Stevie tension |
| 0105.png | 1:05 | Bit 1 | Piano player + bassist behind | Christine & John |
| 0110.png | 1:10 | Bit 1 | Two wedding rings on surface | Marriages intact — for now |
| 0115.png | 1:15 | Bit 1 | Vinyl pressing factory, records rolling off | Album goes to press |
| 0120.png | 1:20 | Bit 1 | Close-up of spinning record label | Rumours releases |
| 0125.png | 1:25 | Bit 1 | Crowd outside packed concert venue | Sell-out shows begin |
| 0130.png | 1:30 | Bit 1 | Champagne, cash, celebration | Commercial explosion |
| 0135.png | 1:35 | Bit 1 | Golden group portrait, all five | Peak — the perfect picture |
| 0140.png | 1:40 | Bit 1 → Bit 2 | Crack running across stage floor | The fracture begins |
| 0145.png | 1:45 | Bit 2 | Two couples with visible rift between them | Breakups crystallize |
| 0150.png | 1:50 | Bit 2 | Dawn on highway, lone figure walking | Someone leaving |
| 0155.png | 1:55 | Bit 2 | Hunched silhouette backstage | Backstage isolation |
| 0200.png | 2:00 | Bit 2 | Couple drifting apart, backs to each other | Lindsey & Stevie split |
| 0205.png | 2:05 | Bit 2 | Guitar left leaning against wall, figure at door | Departure / walking away |
| 0210.png | 2:10 | Bit 2 | Torn photograph | Memories torn apart |
| 0215.png | 2:15 | Bit 2 | Divorce papers on desk | John & Christine divorce |
| 0220.png | 2:20 | Bit 2 | Two silhouettes in same room, backs turned | Forced coexistence |
| 0225.png | 2:25 | Bit 2 | Woman at piano, bassist standing behind | Christine & John in session |
| 0230.png | 2:30 | Bit 2 | Old phone off hook, dial tone implied | Cut off, no communication |
| 0235.png | 2:35 | Bit 2 | Head in hands at desk | Emotional collapse |
| 0240.png | 2:40 | Bit 2 | Door ajar, two shadows in doorframe | On the edge of leaving |
| 0245.png | 2:45 | Bit 2 | Wilting roses on table | Love dying |
| 0250.png | 2:50 | Bit 2 | Three couples dissolving into darkness | All three relationships end |
| 0255.png | 2:55 | Bit 2 → Bit 3 | Split silhouette, one person becomes two | Fracture complete |
| 0300.png | 3:00 | Bit 3 | Hands gripping mixing console edge | Back in the studio — no choice |
| 0305.png | 3:05 | Bit 3 | Clock face, time passing, late night | Endless sessions |
| 0310.png | 3:10 | Bit 3 | Woman's silhouette alone at microphone | Singing through pain |
| 0315.png | 3:15 | Bit 3 | Dark empty control room, single lamp | Late-night isolation |
| 0320.png | 3:20 | Bit 3 | Rain on studio window, two cups of cold coffee | Two people, no words |
| 0325.png | 3:25 | Bit 3 | Single lamp in total darkness | Rock bottom |
| 0330.png | 3:30 | Bit 3 | Studio building exterior by the bay, dawn | Sausalito — the place that held them |
| 0335.png | 3:35 | Bit 3 | Bridge in morning fog | The long way through |
| 0340.png | 3:40 | Bit 3 | Clock hands at 4am | Another all-nighter |
| 0345.png | 3:45 | Bit 3 | Exhausted silhouette at mixing board, coffee & ashtray | The album finishes itself |

---

---

## REAL AUDIO TIMING (ElevenLabs VO = 5:21 total)

The recorded voiceover came out **5:21**, faster than the script estimate. Real section anchors (from the audio):

| Section | Real timecode |
|---------|--------------|
| Hook → Bit 3 (Sausalito) | 0:00 → 3:10 |
| Bit 4 (the songs / letters) | 3:10 → 4:03 |
| Bit 5 (Mick & Stevie affair) | 4:03 → 4:26 |
| Climax (40 million) | 4:26 → 4:58 |
| Outro / CTA | 4:58 → 5:21 |

**Placement rule:** Filenames keep sort order (MMSS grid). The 46 original frames (0000–0345) get compressed into **0:00–3:10**; the 22 tail frames (0350–0535) span **3:10–5:21**. In CapCut, place each frame inside its section window and match the spoken line — don't force a rigid 5s.

---

## TAIL FRAMES (0350–0535) — Bit 4 → Outro

| File | ~Place at | Section | Scene |
|------|-----------|---------|-------|
| 0350.png | 3:10 | Bit 4 | Hand on tape reel, lyric sheet — "turned pain into songs" |
| 0355.png | 3:17 | Bit 4 | Guitar-neck silhouette, backlit — "Go Your Own Way" |
| 0400.png | 3:23 | Bit 4 | Female silhouette at mic, bowed — Stevie backing vocals |
| 0405.png | 3:30 | Bit 4 | Red room + bed — "Dreams" / Sly Stone's studio |
| 0410.png | 3:37 | Bit 4 | Calm vs tense split by doorway — quiet revenge vs rage |
| 0415.png | 3:43 | Bit 4 | Two figures, red stage light — "You Make Loving Fun" affair |
| 0420.png | 3:50 | Bit 4 | Bassist's hands on strings — John playing bass |
| 0425.png | 3:56 | Bit 4 | Dog in pool of light — "the song was about her dog" |
| 0430.png | 4:03 | Bit 5 | Bare bulb, cracked wall — "couldn't get worse" |
| 0435.png | 4:09 | Bit 5 | Two shadows merging on wall — Mick & Stevie secret affair |
| 0440.png | 4:15 | Bit 5 | Cracked wooden beam — "last support beam, one more crack" |
| 0445.png | 4:20 | Bit 5 | Scattered dim studio — "nobody knew who was with whom" |
| 0450.png | 4:26 | Climax | Vinyl sliding from plain sleeve — Feb 4 1977 release |
| 0455.png | 4:31 | Climax | Turntable, needle dropping — "they called it Rumours" |
| 0500.png | 4:37 | Climax | City night, lit windows — "world heard their breakups" |
| 0505.png | 4:42 | Climax | Pressing plant, record stacks — "forty million copies" |
| 0510.png | 4:47 | Climax | Award statuette silhouette — "Album of the Year" |
| 0515.png | 4:52 | Climax | Tape reel in golden light — "bought it 40 million times" |
| 0520.png | 4:58 | Outro | Gold records wall, one empty — "made them immortal" |
| 0525.png | 5:05 | Outro | Wilting rose by lamp — "the wounds were real" |
| 0530.png | 5:11 | Outro | Dawn through studio window — "born when life falls apart" |
| 0535.png | 5:16 | Outro | Dying lamp + console — CTA, mirrors opening 0000 (ring closes) |

**Replaced frames:** 0140 (was glitch crack → now two silhouettes back-to-back) and 0250 (was broken glass/faces → now three pairs dissolving in smoke).

---

## CapCut Import Checklist

1. Sort files by name (MMSS = alphabetical = chronological order) — all 68 frames
2. Select all → drag to timeline
3. Original 46 (0000–0345) → spread across **0:00–3:10** (~4s each); tail 22 (0350–0535) → **3:10–5:21** (~6s each)
4. Fine-tune each frame's position to match its spoken line (see tables above)
5. Video length follows the 5:21 voiceover — no extra runtime to fill
6. Apply slow Ken Burns (pan/zoom) to every still

## Narrative Block Summary (real audio)

| Block | Real timecode | Content |
|-------|---------------|---------|
| Hook | 0:00–0:18 | Studio + broken relationships established |
| Open Loop | 0:18–0:35 | The impossible premise — sing about each other |
| Bit 1 | 0:35–~1:45 | Success, fame, the perfect picture |
| Bit 2 | ~1:45–~2:50 | All three relationships collapse |
| Bit 3 | ~2:50–3:10 | Back to the studio (Sausalito pressure cooker) |
| Bit 4 | 3:10–4:03 | The songs / letters (loop payoff) |
| Bit 5 | 4:03–4:26 | Mick & Stevie affair |
| Climax | 4:26–4:58 | 40 million copies |
| Outro | 4:58–5:21 | Immortal + CTA |

---

## ANIMATED CLIPS (videos/ — Kling 3.0 image-to-video, 5s, silent)

15 key frames animated. In CapCut, **replace the matching still with the .mp4** at that timecode; all other frames stay as stills with Ken Burns.

| Clip | Res | Motion |
|------|-----|--------|
| videos/0000.mp4 | 1080p (pro) | Reel rotates, smoke drifts, lamp flickers — HERO opener |
| videos/0005.mp4 | 720p | Subtle drift, smoke, silhouettes barely shift |
| videos/0010.mp4 | 720p | Slow push-in to lone figure at board |
| videos/0015.mp4 | 720p | Camera drift across live room, dust |
| videos/0020.mp4 | 720p | Micro push-in on microphone, haze |
| videos/0025.mp4 | 720p | Hand keeps writing on paper |
| videos/0030.mp4 | 720p | VU meter needle trembles |
| videos/0035.mp4 | 720p | Crowd sways, stage lights flare |
| videos/0040.mp4 | 720p | Tour bus moves along night highway |
| videos/0120.mp4 | 720p | Vinyl record spins |
| videos/0435.mp4 | 720p | Two shadows drift closer — Mick & Stevie affair |
| videos/0455.mp4 | 720p | Turntable spins, needle in groove |
| videos/0505.mp4 | 720p | Camera drift through pressing plant |
| videos/0515.mp4 | 1080p (pro) | Tape reel in golden light — HERO climax |
| videos/0535.mp4 | 1080p (pro) | Reel's last rotation, lamp dims — HERO closer (mirrors 0000) |

Source model: `kling3_0`, sound off, started from each frame's Higgsfield job_id. Cost ≈ 7.5 cr/clip (std), pro higher.
